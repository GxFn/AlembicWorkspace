...
const exchangePath = path.join(workspaceRoot, "docs/workspace/current/alembic-test-exchange.md");
...
if (planPath && existsSync(planPath)) {
  ...
  const alembicTestRows = dispatchRows.filter(
    (row) => row.windowName === "AlembicTest" && sendEligibleStatuses.has(row.status),
  );
...
if (existsSync(exchangePath)) {
  ...
  warnings.push("docs/workspace/current/alembic-test-exchange.md is missing.");
}

if (alembicTestSendEligible && activeTests.length === 0 && !alembicTestNonTestOnly) {
  issues.push("AlembicTest is send-eligible in the current plan, but no active AlembicTest test card exists.");
}
...
if (json) {
  ...
  console.log(`AlembicTest send-eligible: ${alembicTestSendEligible ? "yes" : "no"}`);
  console.log(`AlembicTest non-test task: ${alembicTestNonTestOnly ? "yes" : "no"}`);
  console.log(`Active AlembicTest tests: ${activeTests.length}`);
