...
| Workspace skill `alembic-workspace-control` | Procedures that are needed only for a matching workflow: TODO, dispatch, testing, script pipeline, ledger moves, and this architecture map. | Hard anti-failure rules that were added because total control repeatedly made mistakes. |
...
2. Would hiding it make it easier to skip thinking, over-delegate, over-document,
   trust a window self-report, misuse `AlembicTest`, or fabricate progress?
...
| --- | --- | --- | --- |
| Controller self heartbeat | The current task is owned by `AlembicWorkspace` itself. | Codex heartbeat attached to the current thread; no VAD target queue. | User stop, plan hard gate, or no useful next unit. |
