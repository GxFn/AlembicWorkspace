...
Use this reference when creating, adjusting, accepting, rolling, canceling, prioritizing, or dispatching TODO / Backlog items in AlembicWorkspace.

## Core Rules

- TODO / Backlog 是通用子模式，不只用于问题修复。用户可以在需求讨论、需求设计、代码调研、目标阶段确认、wave 派发、监控、验收或真实测试期间要求新增、调整、取消或重排 TODO。
- `AlembicDesign` signal 不是正式 TODO；总控接收后才决定是否写入 `global-todo-board`、当前 wave `TODO / Backlog`、测试交流或需求目录。
- `AlembicDesign` 完成需求设计并设定目标后，默认不再作为零散交流处理，而应作为正式 TODO / Backlog 候选交回总控。总控要把它挂到正确账本，再按当前主线是否完成、优先级、依赖和目标阶段确认正常领取推进；除非用户明确要求打断当前主线，否则不得因 Design 完成而直接跳过 TODO 或立即派发。
