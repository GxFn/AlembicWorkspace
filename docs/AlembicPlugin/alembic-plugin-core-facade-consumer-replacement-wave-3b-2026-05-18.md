...
| `npm run build:check` | 本次复跑未通过；当前 `../AlembicCore` 工作区存在未提交改动，`src/types/index.ts` 因重复导出 `IncrementalPlan` 阻断 Core build。此前提交 `f185e95127411b9b6fcac6df43709be9b1ccee54` 时使用 Core `75fac5642b6da736a00667539a720172d23b85c3` 通过。 |
...
| 目标旧 specifier 负向扫描 | 通过；除 `@alembic/core/types/workflows` Core blocker 外 0 命中。 |
...
- 本波目标旧 deep specifier 负向扫描：除 Core blocker `@alembic/core/types/workflows` 外已清零。
- 非阻塞 residual：`@alembic/core/shared/recipe-tokens` 已清零。
- Core 缺口 residual：`@alembic/core/types/workflows` 1 命中，等待 Core facade 补齐后再删除。
...
- 当前 `../AlembicCore` 工作区有 3B-Core-2 相关未提交改动，`src/types/index.ts` 重复导出 `IncrementalPlan` 导致 Plugin `npm run build:check` 复跑失败；需要 Core 窗口先修复并提交。
