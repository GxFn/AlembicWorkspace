...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TODO-PKS-1 | 已完成 | 并行修复 / 交互契约 | P0 | `AlembicCore` / `AlembicPlugin` | Recipe 产出后的 Mission Briefing 仍提示旧工具名 `knowledge({ action: "submit" })` / `submit_batch`，需要统一为真实暴露的 `alembic_submit_knowledge`，避免 Codex host agent 被错误 next step 带偏。 | 否 | Recipe 交互契约 Wave 1 已完成：Core 提交 `bd9319db72d6fd22f9b3a2ba3a36e279ee117f24`。 | `AlembicCore` / `AlembicPlugin` |
| TODO-PKS-2 | 已完成 | 并行修复 / 交互契约 | P0 | `AlembicPlugin` / `AlembicCore` | `alembic_submit_knowledge` 返回 `pendingSemanticReview` 时，建议的 `alembic_consolidate` nextAction 中 `newRecipeId` 为空；需要让返回数据能直接关联新 Recipe ID，形成可执行的合并 / 拒绝闭环。 | 否 | Recipe 交互契约 Wave 1 已完成：Core 提供 `newRecipeId` / `createdRecipe`，Plugin 提交 `8602ae9e71874af389709db680104b2c1ee0edbb` 消费。 | `AlembicPlugin` |
| TODO-PKS-3 | 已完成 | 并行策略确认 | P1 | `AlembicCore` / `AlembicPlugin` | Codex 提交 source 已规范化为 `host-agent`，但 ConfidenceRouter 默认 trusted sources 不包含 `host-agent`；需要确认 Codex host-agent Recipe 应走更高阈值还是纳入受信来源。 | 否 | Recipe 交互契约 Wave 1 已完成：`host-agent` 已纳入 trusted source 且保留内容 / reasoning / quality / confidence gates。 | `AlembicCore` |
| TODO-PKS-4 | 已完成 | 并行修复 / 契约收敛 | P1 | `AlembicPlugin` | lifecycle schema / tool 描述看起来覆盖 publish / deprecate 等动作，但当前 handler 默认只开放有限 admin 操作；需要收敛 Codex agent 可见文案，避免误以为默认 tier 可直接发布 / 废弃 Recipe。 | 否 | Recipe 交互契约 Wave 1 已完成：默认 Codex agent 可见 lifecycle schema 只开放 `reactivate`。 | `AlembicPlugin` |
| TODO-PKS-5 | 已完成 | 并行优化 / 契约清晰度 | P2 | `AlembicPlugin` | V1 用 `nextActions[].tool = codex_host_response` 表达“宿主可见回复动作”。这能完成呐喊闭环，但 V2 应把 host response action 和真实 MCP tool call 区分得更清楚，避免 Codex 把它误当成可调用工具。 | 否 | Recipe 交互契约 Wave 1 已完成：prime payload 使用 `hostResponse`，`nextActions` 不再暴露虚构 `codex_host_response`。 | `AlembicPlugin` |
...
- `AlembicWorkspace` 总控验收：MAIN-PKS-1 / MAIN-PKS-2 已通过，V1 最小闭环关闭。
  ...
  - 遗留风险：`nextActions[].tool = codex_host_response` 是 host-response 语义而非真实 MCP tool；V1 接受，并已在 Recipe 交互契约 Wave 1 中完成收敛。
- 2026-05-21：TODO-PKS-1 至 TODO-PKS-5 已由 [alembic-codex-recipe-interaction-contract-wave-2026-05-21.md](alembic-codex-recipe-interaction-contract-wave-2026-05-21.md) 关闭。总控验收通过：Core 输出统一为 `alembic_submit_knowledge`，pending semantic review 有真实 `newRecipeId` / `createdRecipe`，`host-agent` trusted 策略保留质量门，Plugin lifecycle 默认只开放 `reactivate`，prime host response 已从 MCP tool 语义中分离。
