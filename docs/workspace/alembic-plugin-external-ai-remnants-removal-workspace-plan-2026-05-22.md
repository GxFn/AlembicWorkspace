...
- `AlembicPlugin` 只在确有真实消费方时保留 AI 配置 / 状态 / 权限 / 边界提示，不保留可被误解为可执行 provider runtime 的 `chat()` / `embed()` 外壳。
...
- `AlembicPlugin/lib/codex/HostAiAdapter.ts:97` 仍维护 Google / OpenAI / DeepSeek / Claude / Ollama provider config、默认模型、key env var 和 baseUrl；这部分可能仍服务配置状态，但不应被误读为 Plugin 内置执行 provider。
...
- `AlembicPlugin/lib/http/routes/ai.ts:1` 已声明 Plugin 不再拥有 local AI execution，`:64` 的执行类接口会返回 501 `HOST_AI_MANAGED`；但 `:151` / `:200` / `:385` 仍提供 providers、config、workspace-config 与 embed provider/env 写入 surfaces。
- `AlembicPlugin/lib/codex/ToolPolicy.ts:138` 仍暴露 `alembic_codex_ai_config` tool，描述为 Alembic internal bootstrap/rescan daemon jobs 使用；这可能是应保留的 resident/internal 配置入口，而不是 Plugin 自己的 AI runtime。
- `AlembicPlugin/lib/external/mcp/CodexMcpServer.ts:376` 的 `configureAi()` 仍能读写 `ALEMBIC_AI_PROVIDER` / `ALEMBIC_AI_MODEL` / key env 等 workspace config；这条链路需要确认是否应该继续作为 Alembic resident/internal job 配置入口，或改名以降低 Plugin 内置 AI 误解。

## 当前判断

删除方向成立，但必须分两层判断：

- 可删除候选：`AlembicPlugin` 内部可执行 provider 外形、unavailable `chat()` / `embed()` 方法占位、DI `_embedProvider` 误用入口、把 Plugin 搜索 / 索引伪装成可本地执行 embedding provider 的残留。
- 暂不直接删除：真实服务于 Alembic resident/internal bootstrap/rescan 的 AI 配置状态、权限检查、密钥确认、masked status、Dashboard/API 兼容 surfaces。它们是否改名、下沉或转发，需要先证明生产方 / 消费方 / 替代入口。

因此本轮先做 AIP-0 代码依赖调研，再决定是否派发 `AlembicPlugin` 实现窗口。主闭环仍是“删除残留后 Codex prime/search 可继续工作、resident vector 增强仍可用、Alembic internal AI 配置不被误删”。

## 完成功能闭环

输入：

- Codex 用户在 `AlembicPlugin` 中使用 prime / search / init / diagnostics / ai_config / dashboard 等 MCP tool。
...
- Plugin 对 AI config 的保留部分必须明确是 Alembic resident/internal job 配置，或被删除 / 改名 / 转交到 Alembic。
...
- 配置 / 状态 API 不再暗示 Plugin 内置第三方 AI 执行能力。
...
- AI config tool / HTTP routes 的命名、文案、状态和权限语义清楚表达“配置 Alembic resident/internal job”，不是“配置 Plugin 内置 AI runtime”。
...
| --- | --- | --- | --- | --- | --- |
| AIP-0 | 执行中 | `AlembicWorkspace` | 完成真实调用方扫描和删除边界设计，列出保留 / 删除 / 改名 / 下沉候选。 | 更新本计划的代码调研、窗口状态和下一阶段派发条件。 | 否，总控内执行 |
| AIP-1 | 暂停 | `AlembicPlugin` | 按 AIP-0 结论实现 provider runtime 外形收缩、DI provider 注入收敛和文案边界修正。 | AlembicPlugin commit、执行记录、targeted tests。 | 待 AIP-0 |
| AIP-2 | 暂停 | `Alembic` / `AlembicCore` / `AlembicDashboard` | 只有当 AI config 状态需要迁移、共享 contract 或 Dashboard API 文案同步时启动。 | 对应仓库提交和消费方验证；无必要则明确无任务。 | 待 AIP-1 设计 |
| AIP-3 | 暂停 | `AlembicTest` | 用户需要真实 Codex / BiliDili 验证时，创建测试单验证 prime/search/ai_config 边界。 | `alembic-test-exchange.md` 测试单与回填证据。 | 待产品变更 |
...
| 窗口 / 状态 | 任务 |
| --- | --- |
| `Alembic`<br>观察中 | 可能保留 Alembic resident/internal AI 配置和 embedding 服务能力；AIP-0 前不派发，避免把 Plugin 残留清理误变成 Alembic 迁移。 |
...
| `AlembicDashboard`<br>观察中 | 可能消费 HTTP `/ai/*` config surfaces；AIP-0 确认是否需要文案 / API 状态同步前不派发。 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AIP-TODO-1 | 执行中 | 代码调研 | P0 | `AlembicWorkspace` | 扫描 `HostAiAdapter`、`AiModule`、`KnowledgeModule`、HTTP `/ai/*` routes、MCP `alembic_codex_ai_config`、tests 和 release boundary，确认删除 / 保留 / 改名候选。 | 是 | 当前文档创建后立即执行；完成前不派发实现窗口。 | `AlembicWorkspace` |
| AIP-TODO-2 | 待启动 | 删除实现 | P0 | `AlembicPlugin` | 删除或重命名可执行 provider runtime 外形，收敛 `_embedProvider` 注入，让 Plugin 不再暴露旧内置第三方 AI 执行残留。 | 是 | 等 AIP-TODO-1 产出明确删除边界和验证命令。 | `AlembicPlugin` |
| AIP-TODO-3 | 观察中 | service/config 边界 | P1 | `Alembic` / `AlembicPlugin` | 判断 AI config 是否保留在 Plugin tool 中作为 Alembic resident/internal job 配置，或需要转交 Alembic service API。 | 可能 | AIP-TODO-1 发现真实消费方或命名歧义后启动。 | 待定 |
| AIP-TODO-4 | 观察中 | UI/API 同步 | P2 | `AlembicDashboard` | 若 HTTP `/ai/*` config surfaces 文案或 schema 改名，确认 Dashboard 是否需要同步。 | 可能 | AIP-TODO-2 触发 API 可见变化时启动。 | `AlembicDashboard` |
| AIP-TODO-5 | 暂停 | 真实项目验证 | P1 | `AlembicTest` | 在产品变更完成后验证 Codex prime/search 仍可用、receipt shout 仍由 Codex 发声、AI config 不再误导为 Plugin 内置执行。 | 是 | 等产品仓库提交和总控创建测试单；当前不启动。 | `AlembicTest` |

## 空闲窗口调度

| 窗口 | 调度 | 是否发送 | 原因 |
| --- | --- | --- | --- |
| `Alembic` | 观察 | 否 | resident/internal AI 配置可能相关，但上游调研未完成。 |
| `AlembicCore` | 观察 | 否 | 暂无共享 contract 变更证据。 |
| `AlembicAgent` | 无任务 | 否 | 本主线不改 Agent runtime。 |
| `AlembicDashboard` | 观察 | 否 | 等 HTTP/API 可见变化明确后再判断。 |
...
当前不向任何执行窗口发送提示词。AIP-0 完成前，`AlembicPlugin` 不应提前删除 `HostAiAdapter` 或 AI config surfaces。
