...
| 类型 | 文档 | 状态 | 说明 |
| --- | --- | --- | --- |
| 当前计划 | [alembic-0-2-0-version-unification-workspace-plan-2026-05-22.md](alembic-0-2-0-version-unification-workspace-plan-2026-05-22.md) | V020-2 / V020-3 待启动 | 启动 `GTODO-2026-05-22-011`：把 Alembic 自有 package / plugin / release / runtime 版本位统一为 `0.2.0`，并在下游完成后刷新 Codex plugin cache。上游版本源已通过总控复核，当前发送 `Alembic` / `AlembicPlugin`。 |
| 当前状态 | [workspace-current-status.md](workspace-current-status.md) | V020-2 / V020-3 待启动 | 当前主线切换到 0.2.0 版本统一；`AlembicCore` / `AlembicAgent` / `AlembicDashboard` 已完成，当前发送 `Alembic` / `AlembicPlugin`。 |
...
| 窗口 / 状态 | 任务 |
| --- | --- |
| `Alembic`<br>待启动 | V020-2：更新 `alembic-ai@0.2.0`、lockfile 和 publish staging，确保读取 Core / Agent / Dashboard 的 `0.2.0`。 |
...
| `AlembicPlugin`<br>待启动 | V020-3：更新 Codex plugin manifest / channel / runtime / tests / MCP metadata 到 `0.2.0`；本机 cache refresh 留给总控 V020-4。 |
