...
| 类型 | 文档 | 状态 | 说明 |
| --- | --- | --- | --- |
| AlembicCore facade readiness Wave 3B-Consumer | [alembic-core-facade-readiness-wave-3b-consumer-plan-2026-05-18.md](alembic-core-facade-readiness-wave-3b-consumer-plan-2026-05-18.md) | 执行中 | Core Wave 3B-Core 已总控复核通过，提交 `75fac5642b6da736a00667539a720172d23b85c3`；Alembic 已完成可安全替换的 consumer imports 并提交 `64f30f68ffce13c350ca9c328e511e087ded3246`；AlembicPlugin 已完成可安全替换的 consumer imports 并提交 `f185e95127411b9b6fcac6df43709be9b1ccee54`；`types/workflows` 因 Core `@alembic/core/types` facade 缺口仍阻塞，Plugin 另发现 `@alembic/core/search` individual signal facade 缺口；不触碰 Core 删除、vendor、portable runtime 或 release。 |
| Alembic Core facade consumer replacement Wave 3B | [alembic-core-facade-consumer-replacement-wave-3b-2026-05-18.md](../Alembic/alembic-core-facade-consumer-replacement-wave-3b-2026-05-18.md) | 阻塞 | Alembic 已把 `shared / config / candidate / reactive-evolution / snapshot-views` 目标旧 deep specifier 收敛到 Core exact facades，并收紧 `config/core-import-boundary.json`；`types/workflows` 因 Core facade 缺口保留 9 个代码 type imports + 1 个 boundary allowlist；提交 `64f30f68ffce13c350ca9c328e511e087ded3246`。 |
| AlembicPlugin Core facade consumer replacement Wave 3B | [alembic-plugin-core-facade-consumer-replacement-wave-3b-2026-05-18.md](../AlembicPlugin/alembic-plugin-core-facade-consumer-replacement-wave-3b-2026-05-18.md) | 阻塞 | Plugin 已把可安全替换的 `shared / config / candidate / reactive-evolution / snapshot-views` 目标旧 deep specifier 收敛到 Core exact facades，并收紧 `config/core-import-boundary-allowlist.json`；`types/workflows` 因 Core facade 缺口保留 1 个代码 type import；`SearchRanking` 暴露 Core search individual signal facade 缺口；提交 `f185e95127411b9b6fcac6df43709be9b1ccee54`。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| `Alembic` | 阻塞 | 已完成可安全替换的 Wave 3B consumer imports，并提交 `64f30f68ffce13c350ca9c328e511e087ded3246`；`types/workflows` 因 Core `@alembic/core/types` facade 缺口保留 residual，等待 Core 3B-Core-2 后再收口。 | 已新建 | `docs/Alembic/alembic-core-facade-consumer-replacement-wave-3b-2026-05-18.md` | AlembicCore facade readiness Wave 3B-Consumer“窗口分派” | AlembicCore facade readiness Wave 3B-Consumer“回填区 / Alembic” |
| `AlembicCore` | 待启动 | 需追加 3B-Core-2：让 `@alembic/core/types` re-export `types/workflows.ts` 中的 workflow contract 类型，并补齐或明确 `@alembic/core/search` individual signal public contract；此前 `@alembic/core/types/workflows` consumer residual 不能删除。 | 新建 | `docs/AlembicCore/alembic-core-types-workflows-facade-wave-3b-core-2-2026-05-18.md` | AlembicCore facade readiness Wave 3B-Consumer“回填区 / AlembicCore” | 待新建 Core 3B-Core-2 回填区，并回填当前 Wave 3B-Consumer 文档 |
| `AlembicAgent` | 观察中 | Agent 当前 Core imports stable-only，本波无直接任务；只需后续总控确认 Core public API 没影响 Agent。 | 无需新建 | 无 | AlembicCore facade readiness Wave 3B-Core“窗口分派” | AlembicCore facade readiness Wave 3B-Core“回填区 / AlembicAgent” |
| `AlembicDashboard` | 无任务 | 本波不涉及 Dashboard API client 或 UI。 | 无需新建 | 无 | AlembicCore facade readiness Wave 3B-Consumer“窗口分派” | AlembicCore facade readiness Wave 3B-Consumer“回填区 / AlembicDashboard” |
| `AlembicPlugin` | 阻塞 | 已完成可安全替换的 Wave 3B consumer imports，并提交 `f185e95127411b9b6fcac6df43709be9b1ccee54`；`types/workflows` 因 Core `@alembic/core/types` facade 缺口保留 1 个 residual；`SearchRanking` 还暴露 Core `@alembic/core/search` individual signal facade 缺口。 | 已新建 | `docs/AlembicPlugin/alembic-plugin-core-facade-consumer-replacement-wave-3b-2026-05-18.md` | AlembicCore facade readiness Wave 3B-Consumer“窗口分派” | AlembicCore facade readiness Wave 3B-Consumer“回填区 / AlembicPlugin” |
| `BiliDili` | 无任务 | 当前是 Alembic / AlembicPlugin Core import 收敛，不涉及真实测试项目。 | 无需新建 | 无 | AlembicCore facade readiness Wave 3B-Consumer“窗口分派” | AlembicCore facade readiness Wave 3B-Consumer“回填区 / BiliDili” |
