...
| 类型 | 文档 | 状态 | 说明 |
| --- | --- | --- | --- |
| 当前计划 | [prime-immediate-receipt-shout-workspace-plan-2026-05-21.md](prime-immediate-receipt-shout-workspace-plan-2026-05-21.md) | AlembicTest 待启动 | prime 后立即呐喊时序已通过 BiliDili 验证；`AlembicPlugin` SHOUT-5 已通过总控验收并刷新本机 Codex plugin cache，当前派发 `AlembicTest` 做可见摘要复测。 |
| 当前状态 | [workspace-current-status.md](workspace-current-status.md) | AlembicTest 待启动 | Test-2026-05-21-04 已创建，验证 SHOUT-5 后的 Codex 可见呐喊是否主动、有声量，且不默认倾倒 evidenceRefs。 |
...
| 当前测试单 | [alembic-test-exchange.md](alembic-test-exchange.md) | AlembicTest 待启动 | Test-2026-05-21-04：BiliDili prime readable receipt shout 可见摘要复测。 |
...
| AlembicTest 测试交流文档 | [alembic-test-exchange.md](alembic-test-exchange.md) | 待启动 | Test-2026-05-21-04 已创建，发送给 `AlembicTest` 做 BiliDili prime readable receipt shout 可见摘要复测。 |
...
| `AlembicTest`<br>待启动 | Test-2026-05-21-04：在 BiliDili 真实项目中验证 SHOUT-5 可见摘要效果，重点看 prime 后下一条可见响应是否主动、有声量，且不默认倾倒 evidenceRefs。 |
