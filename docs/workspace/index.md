...
| 窗口 | 状态 | 任务 |
| --- | --- | --- |
| `AlembicCore` | 待启动 | 追加 3B-Core-2：让 `@alembic/core/types` re-export `types/workflows.ts` workflow contract 类型，并补齐或明确 `@alembic/core/search` individual signal public contract。 |
| `AlembicPlugin` | 阻塞 | 已清理 `test/unit/ContentImpactAnalyzer.test.ts` 的 `@alembic/core/shared/recipe-tokens` residual，补丁提交 `8f48fd1d2b56e0136919414d68e7da93b1707141`；继续等待 Core 3B-Core-2，不硬绕路删除 `@alembic/core/types/workflows` residual。 |
| `Alembic` | 阻塞 | 已完成可安全替换的 Wave 3B consumer imports；等待 Core 3B-Core-2 后再收口 `@alembic/core/types/workflows` residual。 |
| `AlembicAgent` | 观察中 | Agent Core imports stable-only；消费层完成后总控只复核 scan。 |
| `AlembicDashboard` | 无任务 | 本波不涉及 Dashboard API client 或 UI。 |
