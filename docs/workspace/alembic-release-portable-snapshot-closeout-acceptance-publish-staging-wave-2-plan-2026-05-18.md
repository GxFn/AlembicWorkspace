...
| `Alembic` | 已完成 | 已作为 `alembic-ai` owner 新增 publish staging manifest / pack preview。开发 `package.json` 继续 local-source-first；staging manifest 使用 registry `@alembic/core@0.1.0` 与 `@alembic/agent@0.1.0`，并记录 Core / Agent source commits；release workflow 发布 staging package，而不是 dev root。 | 已新建 | `docs/Alembic/alembic-ai-publish-staging-wave-2-2026-05-18.md` | 本文“窗口分派”；Alembic AI publish staging Wave 2 执行记录 | 本文“回填区 / Alembic” | `npm run build:check`；`npm run check`；`npm run build`；`npm run release:package-guard` 预期失败；`npm run release:staging:pack`；workflow YAML parse；静态扫描；`git diff --check` | 已提交 `7f68d43e019db597c52a5a36d64d68d6dfbc6bcf`；真实 publish 仍依赖 Core / Agent registry 可安装、npm 权限和版本唯一性。 |
| `AlembicPlugin` | 已完成 | 已退出 root npm registry publish 链路，改为只产出 Codex 插件产物和 portable runtime artifact。root `package.json` 已 private，`prepublishOnly` 指向 disabled guard；release workflow 删除 root registry publication，改为验证并上传 Codex plugin artifacts；channel package registry 改为 `portable-artifact`；embedded runtime 保留 `file:vendor/AlembicCore` 和 source metadata。 | 已新建 | `docs/AlembicPlugin/alembic-plugin-artifact-release-no-npm-wave-2-2026-05-18.md` | 本文“窗口分派” | 本文“回填区 / AlembicPlugin” | `npm run build:check`；`npm run build`；`npm run prepare:codex-plugin-runtime`；`npm run verify:codex-plugin`；`npm run verify:codex-channel`；`npm run verify:release-package-boundary`；`node scripts/verify-release-package-boundary.mjs --publish` 预期失败；`npm run smoke:codex-plugin`；`npm run check`；focused CodexMcpServer unit；workflow YAML parse；负向扫描 | 已提交 `6883affe2668c33627aa5c9529e12096735f3abe`；子仓库提交 `dbbeb3a2da170256ed09b69b82771c6db1c7acf5`；真实 artifact upload 仍依赖 GitHub Release workflow 执行。 |
...
- 状态：已完成
- 完成范围：保留 root dev manifest 的 local-source-first 入口 `@alembic/core: file:../AlembicCore` 与 `@alembic/agent: file:../AlembicAgent`；新增 `scripts/prepare-publish-staging.mjs`；新增 `release:staging:prepare` / `release:staging:pack`；staging manifest 将 Core / Agent 替换为 registry version `0.1.0` 并写入 `alembic-release-source.json`；更新 root release package guard，按总控决策将 `alembic-ai` 视为 Alembic 主仓库拥有，同时继续阻断 root dev package 的 `file:../...` / `file:vendor/...` 发布泄漏；release workflow 改为验证 dev root blocked、生成 staging、预览 staging，并 `npm publish .release/alembic-ai`；本地 `scripts/release.ts` 改为验证 dev root blocked + staging pack preview；新增 `.release/` ignore。
- 提交 hash：`7f68d43e019db597c52a5a36d64d68d6dfbc6bcf`（`chore: stage alembic npm publish package`）
- 验证命令：`npm run build:check`；`npm run check`；`npm run build`；`npm run release:package-guard`；`npm run release:staging:pack`；`node -e "const fs=require('fs'); const yaml=require('yaml'); for (const p of ['.github/workflows/release.yml']) { yaml.parse(fs.readFileSync(p,'utf8')); console.log(p+' ok'); }"`；`rg -n "npm publish\|npm pack --dry-run\|ALEMBIC_MAIN_NPM_PACKAGE_OWNER_CONFIRMED\|file:\.\./Alembic(Core\|Agent)\|file:vendor" package.json package-lock.json scripts .github/workflows`；`git diff --check`；`git diff --cached --check`
- 验证结果：`npm run build:check` 通过并使用 local `../AlembicCore` source；`npm run check` 通过，Biome 仍输出既有 warning/info，Core import boundary 扫描 455 files / 598 imports，issue 0；`npm run build` 通过；`npm run release:package-guard` 预期失败，阻断 root `@alembic/core=file:../AlembicCore`、`@alembic/agent=file:../AlembicAgent` 和 root lockfile local links；`npm run release:staging:pack` 通过，生成 `.release/alembic-ai` staging package，pack preview 为 `alembic-ai@0.1.0`，entryCount 525；staging metadata 记录 Alembic `7f68d43e019db597c52a5a36d64d68d6dfbc6bcf`、Core `9174c5173a7313b916b89b7c605ea2afdd874269`、Agent `f9d020f9ebaf95499bbd6e9afbdecafa0615a865`、Dashboard `7143a7ca610a504b7472ae4afac0eb2df2ebdda8`，且 Alembic dirty 为 `false`；workflow YAML parse 通过；静态扫描确认唯一 workflow publish 入口为 `npm publish .release/alembic-ai ...`；diff check 通过；Alembic 工作区干净。
- 遗留风险：本轮没有真实 npm publish；真实发布仍依赖 `@alembic/core@0.1.0` 和 `@alembic/agent@0.1.0` 已在 registry 可安装、npm token / trusted publishing 权限、版本唯一性和 release tag 策略。当前 `alembic-ai` 版本仍为 `0.1.0`，如果 registry 已有同版本，发布前需要升版本。Release workflow 仍 checkout sibling 默认分支，staging metadata 会记录实际 source commits；严格 release snapshot 如需锁定上游，应在 release orchestration 中 pin ref。`npm run check` 仍打印既有 Biome warning/info，本轮未处理历史 lint warning。
- 下一步建议：等待 `AlembicPlugin` 完成 artifact-only release / root npm publish exit 后，统一复核 Wave 2；进入真实发布前，先按顺序确认 `@alembic/core`、`@alembic/agent` registry 包可安装，再发布 Alembic 主仓库 `alembic-ai` staging package。

### AlembicPlugin

- 状态：已完成
- 完成范围：root `package.json` 标记 `private: true`，`prepublishOnly` 改为 `release:root-npm-publish:disabled`，旧 root publish 脚本移除；`.github/workflows/release.yml` 从 registry publish job 改为 `build-test-artifacts`，保留 build / Dashboard build / runtime prepare / verify / lint / tests / smoke，并通过 `actions/upload-artifact` 上传 Codex plugin artifacts；`verify-release-package-boundary` 改为 artifact-only hard gate，普通 verify 通过、publish mode 永远阻断 root registry publication；`channels/codex/channel.json` 改为 `portable-artifact`，artifact 指向 `plugins/alembic-codex/runtime.tgz`；Codex diagnostics / README / release playbook 删除全局 `alembic-ai` registry fallback 口径；runtime prepare 不再拷贝 vendored Core README，避免把 Core 文档里的 Agent package 文字带进 Plugin portable runtime；刷新 `runtime` 与 `runtime.tgz`。
- 提交 hash：AlembicPlugin `6883affe2668c33627aa5c9529e12096735f3abe`（`chore: disable root registry publish for plugin release`）；`plugins/alembic-codex` `dbbeb3a2da170256ed09b69b82771c6db1c7acf5`（`build: make codex runtime artifact-only`）。
- 验证命令：`npm run build:check`；`npm run build`；`npm run prepare:codex-plugin-runtime`；`npm run verify:codex-plugin`；`npm run verify:codex-channel`；`npm run verify:release-package-boundary`；`node scripts/verify-release-package-boundary.mjs --publish`；`npm run smoke:codex-plugin`；`npm run check`；`npx vitest run --config vitest.unit.config.ts test/unit/CodexMcpServer.test.ts`；workflow YAML parse；root publish / global fallback / Agent package negative scan；`git diff --check`；`git -C plugins/alembic-codex diff --check`。
- 验证结果：通过。`npm run build:check` 与 `npm run build` 通过，Core build 使用 `../AlembicCore` commit `9174c5173a7313b916b89b7c605ea2afdd874269`；`prepare:codex-plugin-runtime` 通过并生成 `runtime.tgz`；`verify:codex-plugin` 通过；`verify:codex-channel` 通过；`verify:release-package-boundary` 通过，root registry publish 为 disabled，embedded runtime Core dependency 为 `file:vendor/AlembicCore`，source metadata commit 为 `9174c5173a7313b916b89b7c605ea2afdd874269`；`--publish` 预期失败并明确阻断 AlembicPlugin root registry publication；`smoke:codex-plugin` 通过，install / stdio / npxRuntime 均 passed；`npm run check` 通过，仍显示既有 Biome warning/info；focused CodexMcpServer unit 29 tests 通过；workflow YAML parse 通过；负向扫描 0 命中；diff check 通过。额外完整 `npm run test:unit` 未作为本轮 gate 通过，失败集中在既有无关 suites，已在专项文档记录。
- 遗留风险：本轮没有真实 GitHub artifact upload；`runtime.tgz` 仍通过 `npx --package ./runtime.tgz` 解析生产依赖，首次运行可能需要网络访问依赖 registry，但这不是 root AlembicPlugin npm package 发布；Release workflow 仍 checkout sibling Core / Dashboard 默认分支，正式冻结源快照时建议由 release orchestration pin refs。
- 下一步建议：Wave 2 可进入总控完成；后续 AlembicPlugin 只维护 Codex plugin artifact、channel / marketplace metadata、portable runtime artifact 和 Codex host adapter，不恢复 root registry package 发布入口。

### AlembicDashboard

- 状态：观察中
- 观察结论：本轮未修改 Dashboard 源码；仅继续作为 AlembicPlugin release workflow sibling build source。无新增任务。

### BiliDili

- 状态：无任务
- 判断理由：本轮是 Alembic release staging / package ownership / Plugin artifact-only release 收口，不涉及 iOS/Swift 测试项目。
