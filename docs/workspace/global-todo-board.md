...
| GTODO-2026-05-22-011 | 执行中 | 版本发布 / 缓存刷新 | P0 | `Alembic` / `AlembicCore` / `AlembicAgent` / `AlembicDashboard` / `AlembicPlugin` / `AlembicWorkspace` | 将 Alembic 自有 package、plugin manifest、channel、release staging、Codex runtime 与本机 plugin cache 版本位统一为 `0.2.0`。 | 是，影响发布、部署和 Codex plugin cache。 | 上游版本源与 Agent lockfile snapshot 已通过总控复核；当前派发 `Alembic` / `AlembicPlugin`，完成后由总控刷新 Codex plugin cache。 | `AlembicWorkspace` | [alembic-0-2-0-version-unification-workspace-plan-2026-05-22.md](alembic-0-2-0-version-unification-workspace-plan-2026-05-22.md) |
