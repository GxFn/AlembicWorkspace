...
进入本 workspace 后，先读取 `AGENTS.md`、`docs/workspace/index.md` 和 `docs/workspace/workspace-current-status.md`，再根据当前总控文档继续工作。
...
1. 当前任务分区是什么，是否真的需要启用完整需求 / wave 流程？
2. 当前主线的下一处真实阻塞点在哪里，阻塞点之前还能安全完成哪些事？
3. 前期分析发现的问题是否已经进入 TODO / Backlog，还是明确说明了不入 TODO 的理由？
4. 当前派发是否应该组成任务包，任务包是否覆盖了同窗口、同边界、同验证链路下可关闭的 TODO？
5. 分派提示词是否要求执行窗口先读取目标仓库 `AGENTS.md`，并明确声明自己当前窗口定位 / 仓库职责？
6. 是否需要 `AlembicTest` 测试交接；如果不需要，原因是否已经写清？

如果总控发现自己刚刚派发了碎片任务、漏记 TODO、没有判断阶段顺序、没有说明阻塞点，必须立刻补 TODO、重排阶段、更新当前计划或说明纠偏；不要在错误节奏上继续推进。

## 任务分区入口

用户通常会指定单一任务。总控先按以下分区选择最小流程，不要把其它分区的动作一并展开：

- **入口同步**：用户要求读取当前状态、确认等待事项或继续总控工作时，只读取 `AGENTS.md`、`docs/workspace/index.md`、`docs/workspace/workspace-current-status.md` 和当前总控文档，输出状态、阻塞、待验收和下一步。
...
- **分配计划**：用户要求“派发任务”“做一轮计划分配”“开始执行分配计划”时，必须先滚动当前 TODO / Backlog，再基于已确认文档和 TODO 依赖做阶段顺序、任务包、窗口覆盖、producer / consumer 依赖判断、分派表和可复制提示词；每个可发送任务必须把“读取目标仓库 `AGENTS.md` 并明确当前窗口定位 / 仓库职责”写成执行前置硬规则；不要重新写需求设计，除非发现确认门禁被触发。
...
- **测试交接**：用户要求真实项目验证、冷启动、复现或回归时，只通过 `docs/workspace/alembic-test-exchange.md` 创建或更新 `AlembicTest` 测试单。
...
确认前，当前总控文档和 `docs/workspace/workspace-current-status.md` 必须保持 `暂停` 或等待确认口径，`发送给` 必须为 `无`；不要输出执行窗口可复制提示词，也不要把候选窗口标为 `待启动`。
...
- 与独立测试验证窗口或真实测试项目强相关的长期测试计划、复现记录和验证报告，优先写到 `AlembicTest/docs/`，并通过 `docs/workspace/alembic-test-exchange.md` 建立总控交流入口。
...
- 分派默认以“任务包”为单位，而不是每次只派一个很小的动作。任务包必须写清：当前阶段目标、包含的主线任务、可一起关闭的 TODO、明确不包含的事项、文件 / 模块边界、依赖前提、统一验证命令和回填要求。
...
```text
先读取 AGENTS.md、docs/workspace/<当前总控文档名>.md，以及你所在窗口/目标仓库的 AGENTS.md；先明确声明当前窗口定位和本轮仓库职责，再按照文档领取并完成分配给你所在窗口的任务；完成后回填完成范围、提交 hash、验证命令、验证结果、遗留风险和下一步建议。
```
...
- 每次新建 / 激活目标阶段确认或 wave 执行计划后，必须运行 `node scripts/verify-workspace-docs.mjs --all-workspace`、`node scripts/check-dispatch-coverage.mjs` 和 `git diff --check`。
