...
- 如果我准备用脚本输出、窗口回填、TODO、状态表或文档模板代替总控判断，停止。AlembicWorkspace 是跨仓库目标、边界、证据和验收的最高判断面，必须时刻保持思考。
...
- 如果我准备把总控或 `AlembicDesign` 主动提出的建议、风险提醒、候选方案、阶段取舍或隐藏目标澄清，直接写成已确认目标、TODO、当前计划、分派任务包或实现范围，而没有判断它是否改变用户原始完成定义、执行范围、仓库边界、阶段顺序或用户可见行为，停止。
- 如果我准备把总控裁决、`AlembicDesign` 建议或 Agent 判断表述成最终产品决定，或忘记最终决定权永远在开发者 / 用户一侧，停止。
...
- 如果我把 sourceRef 来源定位问题错误实现成生产期 gate，或准备拦截、拒绝、重写、补全正确 sourceRef / candidate / Recipe，停止。除非用户明确要求拦截，否则只允许非阻塞展示、诊断和校验；不能影响落库、候选创建或 cold-start / rescan 产出。
...
- **测试交接**：只有真实项目验证、cold-start / rescan、复现、回归、Dashboard 手动观察、运行时监控或跨仓库集成环境证据，才创建 `AlembicTest` 测试边界；新流程先在当前 controller state-root 下写入 `test-cards/*.json`，再由总控决定是否生成任务包和投递。`test-exchange.md` 只作人读摘要 / exchange 投影，不作状态源。
...
测试 card、证据解释和验证命令细节见 `codex-control-workspace/skills/dev/control-workspace-governance/references/testing-validation.md`。
...
- `codex-control-workspace/.workspace-active/workspace/index.md` 是 workspace 级唯一活跃总控入口。当前状态、活跃 TODO、测试 card / 测试交流投影、Design inbox、自动化当前计划和正在执行的 workspace 总控计划优先写到 `codex-control-workspace/.workspace-active/workspace/current/` 或当前 controller state-root；它是本机当前工作面，默认不提交。完成后再归档或提炼到 `workspace-ledger/`。
...
- 即使真实测试项目自身包含 `docs/`，开发协作文件、阶段计划、验收记录、扫描结果和 Alembic 验证记录仍统一通过 controller state-root、workspace 总控文档、内部 `codex-control-workspace/.workspace-active/workspace/current/test-exchange.md` 投影，或外部 `AlembicTest/docs/` 记录；真实测试项目仓库内 `docs/` 只保存必要的长期项目文档。
...
- 正规需求路线是：Design 先完成 original plan、requirement design、目标、完成定义、阶段候选和 TODO / Backlog 挂载建议；总控接收后正式入 TODO / Backlog、需求目录或 controller state-root intake，再决定补代码调研、创建测试 card / 任务包、进入目标阶段确认或启动执行。
...
- 新建或调整状态机需求根、开发者推进文档、Design handoff board、Design/Test intake、测试 card / 测试交流投影、归档入口或相关模板时，必须遵守 `codex-control-workspace/scripts/README.md` 中的脚本可读格式说明。新需求使用 `codex-control-workspace/templates/control-state-machine/` 与 `controller-state.mjs`；Design/Test 附件使用 `control-intake.mjs`；开发者推进文档只允许脚本更新统一状态块，其余补充走追加日志。不要随意重命名脚本依赖章节或改变状态机、target result、reducer candidate、窗口分派和回填证据的机器字段。
...
当用户需要把下一波任务复制到其它 Codex 窗口时，总控窗口默认只输出一条轻量通用提示词，让各窗口根据当前总控文档自行领取分配给自己的任务。提示词是导航和唤醒信封，不是任务书；目标、范围、禁止事项、验证和回填字段必须写在当前计划、测试 card / 任务包、目标仓库 `AGENTS.md` 或相关 skill 中。详细发送 / 不发送判断见 `codex-control-workspace/skills/dev/control-workspace-governance/references/window-dispatch.md`。
...
- 使用地图：
  ...
  - 做脚本维护、脚本验证、Design handoff 导入、controller state root / developer progress 投影和 runtime 检查时，读 `codex-control-workspace/skills/dev/control-workspace-governance/references/script-pipeline.md`；脚本不得替代总控判断仍留在 `AGENTS.md`。
