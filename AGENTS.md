...
4. If there is a current plan, task package, or direct-thread delivery, execute only the content under `.workspace-active/workspace/current` explicitly assigned to `AlembicWorkspace`.
...
> Wakeflow is installed as a Codex plugin for this workspace. Use Wakeflow MCP tools for setup, status, state roots, delivery, review, and verification; local Wakeflow runtime scripts live in the installed plugin, not in this workspace root.
...
Operational details live in `installed Wakeflow skill wakeflow-governance/` and
`installed Wakeflow skill wakeflow-target/`.
...
See `installed Wakeflow skill wakeflow-governance/references/wakeflow-ledgers.md`.
...
- For script maintenance or pipeline questions, read
  `installed Wakeflow skill wakeflow-governance/SKILL.md` and
  `installed Wakeflow skill wakeflow-governance/references/script-pipeline.md`.
- the installed Wakeflow script index is the script index. After adding, renaming, or deleting
  `installed Wakeflow runtime *.mjs`, update the index and run `use the matching Wakeflow MCP tool or installed runtime script for wakeflow-check-scripts.mjs`.
...
- `use the matching Wakeflow MCP tool or installed runtime script for wakeflow-verify.mjs` is the default verification orchestrator.
...
- `installed Wakeflow skill wakeflow-progressive-validation/SKILL.md`: progressive validation
  and source-derived long-chain plans.
- `installed Wakeflow skill wakeflow-target/`: target-window execution and `TargetResultEnvelope`
  backfill.
...
- After creating or activating a phase confirmation or execution wave, run
  `use the matching Wakeflow MCP tool or installed runtime script for wakeflow-verify.mjs`.
- If TODO mode affects dispatch or order, run
  `use the matching Wakeflow MCP tool or installed runtime script for wakeflow-verify.mjs --require-todo`.
- If task packages are used, run
  `use the matching Wakeflow MCP tool or installed runtime script for wakeflow-verify.mjs --require-task-packages`.
- If scripts, script README, or script skills change, run
  `use the matching Wakeflow MCP tool or installed runtime script for wakeflow-verify.mjs --with-script-tests`.
