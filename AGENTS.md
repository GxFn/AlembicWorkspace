...
- VAD 生命周期命令必须语义明确：`cd codex-control-workspace && node scripts/visible-dispatch.mjs init --write --json` 只初始化本地运行态；`cd codex-control-workspace && node scripts/visible-dispatch.mjs start-plan --write --json` 是当前计划首次无人值守启动；`cd codex-control-workspace && node scripts/visible-dispatch.mjs resume-plan --write --json` 是回跳验收后或人工中断后的正常续跑；`cd codex-control-workspace && node scripts/visible-dispatch.mjs stop-plan --write --reason "<reason>"` 是关闭后续自动化跳转。不要使用含糊的旧启动 / 重启别名。
...
- VAD thread id 必须是真实 Codex thread id，只能保存在 `codex-control-workspace/.workspace-local/visible-dispatch/`；不得把 thread id 写入 tracked 文档、GitHub、提示词或回填正文。严禁使用 `current-codex-thread`、`current thread`、`<thread id>`、`unknown`、说明文字或任何占位符登记窗口。

TODO / Backlog、窗口覆盖、任务包和 VAD 命令细节见 `codex-control-workspace/skills/dev/control-workspace-governance/` 与 `codex-control-workspace/skills/dev/visible-automation-dispatch-target/`。
...
- `codex-control-workspace/.workspace-active/workspace/index.md` 是 workspace 级唯一活跃总控入口。当前状态、活跃 TODO、测试交流、Design inbox、VAD 当前计划和正在执行的 workspace 总控计划优先写到 `codex-control-workspace/.workspace-active/workspace/current/`；它是本机当前工作面，默认不提交。完成后再归档或提炼到 `workspace-ledger/`。
...
当用户需要把下一波任务复制到其它 Codex 窗口时，总控窗口默认只输出一条通用提示词，让各窗口根据当前总控文档自行领取分配给自己的任务。详细发送 / 不发送判断见 `codex-control-workspace/skills/dev/control-workspace-governance/references/window-dispatch.md`。

```text
先读取 AGENTS.md、codex-control-workspace/.workspace-active/workspace/index.md、codex-control-workspace/.workspace-active/workspace/current/<当前总控文档名>.md，以及你所在窗口/目标仓库的 AGENTS.md。
...
- 使用地图：
  ...
  - 做 VAD mode / registry / queue / group / heartbeat 操作时，读 `codex-control-workspace/skills/dev/control-workspace-governance/references/visible-automation-dispatch.md`；thread id 真实性、next heartbeat 权限和 `AlembicTest` 边界仍留在 `AGENTS.md`。
...
- `codex-control-workspace/skills/dev/visible-automation-dispatch-target/`：VAD 目标窗口 claim / finish / record-arm / record-stop 命令细节；role guard、thread id 真实性、next heartbeat 权限和 `AlembicTest` 边界必须同时在 `AGENTS.md` 明文常驻。
- `codex-control-workspace/skills/dev/visible-automation-dispatch-controller/`：VAD controller-return heartbeat 的证据复核、下一波决策和避免小任务漂移的操作步骤；总控事实裁决和验收底线仍以 `AGENTS.md` 为准。
