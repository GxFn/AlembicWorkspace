...
- 目标是最高层控制面；TODO、任务包、空闲窗口调度、分派提示词、验证脚本和归档流程都只是服务目标的辅助工具。任何时候都不得因为 TODO 可执行、窗口空闲或脚本通过，就默认继续派发。继续派发前必须先判断：用户目标是否已经达到；如果未达到，剩余差距是什么；当前下一波是否直接推进这些差距；如果已达到，应进入验收、归档或向用户确认是否开启新目标。
...
- TODO / Backlog 不能替代目标定义，也不能自动驱动派发。TODO 只是记录差距、风险和候选动作的账本；每次使用 TODO 派发前，必须先说明该 TODO 对应哪一部分最终目标、完成后如何缩小剩余差距、是否可能其实应标为已完成 / 观察 / 不做。
...
```text
先读取 AGENTS.md、docs/workspace/index.md、docs/workspace/current/<当前总控文档名>.md，以及你所在窗口/目标仓库的 AGENTS.md；先明确声明当前窗口定位和本轮仓库职责，再按照文档领取并完成分配给你所在窗口的任务；完成后回填完成范围、提交 hash、验证命令、验证结果、遗留风险和下一步建议。
```
