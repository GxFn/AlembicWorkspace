...
- `docs/workspace/` 用于跨仓库总控计划、状态快照、分派表、迁移手册、验收汇总、边界判断、窗口调度和指挥中心自己的工作记录。
...
- 给任意窗口分配任务时，必须明确文档要求：
  - 是否需要新建文档，还是更新现有文档，或无需单独文档。
  - 如果新建或更新，必须给出保存路径，例如 `docs/workspace/...`、`docs/AlembicAgent/...`、`docs/AlembicDashboard/...`。
  - 必须说明该文档挂载到哪里：通常挂载到当前总控计划的任务索引、阶段章节或回填模板；子仓库专项文档还要在对应窗口任务中标明它被哪个 workspace 总控文档引用。
  - 如果判断无需新建文档，必须说明原因，并指定完成证据回填到哪个现有文档和哪个章节。
- 文档命名采用稳定格式：
...
- 文档名使用小写 kebab-case；日期使用执行日 `YYYY-MM-DD`；不要把用户本机名、绝对路径、密钥、token 或临时随机串写入文档名。
...
- 任务状态统一使用以下枚举：`待启动`、`执行中`、`待验收`、`阻塞`、`已完成`、`暂停`、`观察中`、`无任务`。
- 状态含义必须稳定：
  - `待启动`：任务已分配但尚未开始。
...
- 窗口覆盖检查默认使用表格字段：`窗口`、`状态`、`任务`、`文档动作`、`保存位置`、`挂载入口`、`回填位置`、`验证命令`、`阻塞/依赖`。
...
- 任务分派中的文档动作必须使用明确字段表达：`文档动作：新建 / 更新 / 无需新建`，`保存位置：...`，`挂载入口：...`，`回填位置：...`。
...
- TypeScript 仓库通常优先运行 `npm run build:check`。
- 需要完整验证时运行 `npm run check`，或按子仓库文档指定组合运行 `npm run lint`、`npm run test`、`npm run build`。
- Plugin 相关改动通常还需要 `npm run smoke:codex-plugin` 和 `npm run verify:codex-plugin`，除非子仓库文档更新了更准确的命令。
- Dashboard 相关改动需要前端构建和必要的浏览器/截图验证。
- BiliDili 相关改动需要按 iOS/Swift 项目验证，优先使用 Xcode scheme `BiliDili` 做模拟器 build/test；若只改文档或总控规则，可说明无需运行 Xcode 构建。
- 如果命令无法运行，必须记录原因，不能把未运行命令写成通过。

## 技术栈与编码约定

- 主要语言：TypeScript，Node.js >= 22。
- 多数包使用 ESM (`"type": "module"`)，新增 import 路径按目标仓库约定处理，通常需要 `.js` 后缀。
- Lint / Format 通常使用 Biome；测试通常使用 Vitest。
- 新增代码应遵守目标子仓库现有结构、package exports、alias 和测试风格。
