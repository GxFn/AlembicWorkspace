...
| 3 | Pinned npm runtime publication (live-proven E404 cold-start blocker; two thin host shells reference it). NAMING RULED 2026-06-13: package renames to **@gxfn/alembic-runtime** BEFORE first publication (zero published consumers — clean window); plugin distribution name → **alembic** (alembic@gxfn) per user ruling + plugin-naming-ruling-2026-06-13.md | originally STAGED by P3 t7 (206-file tgz under the old name); N1 RESTAGED 2026-06-13 under **@gxfn/alembic-runtime@0.2.0** (pack --dry-run: gxfn-alembic-runtime-0.2.0.tgz, 208 files — +2 vs t7 from the P3 t12/P2 ta19 runtime additions). EXACT publish command (user trigger): `cd AlembicPlugin && npm run prepare:codex-runtime-package && cd .tmp/alembic-codex-runtime-package && npm publish --access public` (npm login with @gxfn scope first) | STAGED under final name — user publish trigger |
