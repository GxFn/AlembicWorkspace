...
const baseUrl = process.argv[2] || "http://127.0.0.1:63227";
const outDir =
  process.argv[3] ||
  "/Users/gaoxuefeng/Documents/AlembicWorkspace/.wakeflow-active/current/alembic-panorama-rebuild-2026-07-03/evidence/p4-final-panorama-real-dashboard-scope-rerun-after-rescan-proof-repair-t1";
...
writeFileSync(file("bilidili-rescan-submit.json"), JSON.stringify({ request, response: submit }, null, 2));
...
while (Date.now() - startedAt <= budgetMs) {
  pollIndex += 1;
  const status = await requestJson(`/api/v1/jobs/${jobId}?compact=true`, { timeoutMs: 20_000 });
  const job = compactJob(extractJob(status.data));
  const events = await requestJson(`/api/v1/jobs/${jobId}/events?limit=80`, { timeoutMs: 20_000 });
  const display = await requestJson(`/api/v1/jobs/${jobId}/display-snapshot`, { timeoutMs: 20_000 });
  const jobs = await requestJson("/api/v1/jobs?kind=rescan&limit=5&compact=true", { timeoutMs: 20_000 });
  const record = {
    pollIndex,
  ...
  writeFileSync(file("bilidili-rescan-display-snapshot-latest.json"), JSON.stringify(display.data, null, 2));
